...
export const validateShopCategories = async (): Promise<void> => {
  try {
    console.log('Starting shop category validation...');
    ...
    for (const shop of shops) {
      ...
      if (needsUpdate) {
        try {
          ...
          updatedCount++;
          console.log(`Updated shop ${shop.shopName} category to: ${updates.type}`);
        } catch (error) {
    ...
    console.log(`Shop category validation completed. Updated ${updatedCount} shops.`);
  } catch (error) {
...
export const syncShopCategories = async (): Promise<void> => {
  try {
    console.log('Starting shop category sync...');
    ...
    if (missingCategories.length > 0) {
      console.log(`Found ${missingCategories.length} categories used by shops but not in categories collection:`, missingCategories);
      
      // Note: We don't automatically create categories here to avoid permission issues
      // Instead, we log them for manual review
      console.log('These categories should be manually added to the categories collection by an admin.');
    } else {
      console.log('All shop categories are properly synced with the categories collection.');
    }
