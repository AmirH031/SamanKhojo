...
import { getOptimizedShops } from '../services/optimizedSearchService';
...
const ShopsPage = () => {
  ...
  useEffect(() => {
    const fetchShops = async () => {
      ...
      try {
        ...
        try {
          const backendShops = await getOptimizedShops({
            location,
  ...
  }, [searchTerm, sortBy, shops, searchResults]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 py-4 sm:py-6 md:py-8 mobile-safe-area">
          ...
          >
            <div className="inline-flex items-center space-x-2 bg-green-50 border border-green-200 rounded-lg px-4 py-2">
              ...
              <span className="text-green-800 text-sm font-medium">
                Live Data • {filteredShops.length} Active Shops
              </span>
            </div>
          </motion.div>
        )}
      ...
      >
        <button
          ...
          className="flex items-center space-x-3 bg-green-500 text-white px-6 py-4 rounded-2xl shadow-2xl hover:bg-green-600 transition-all group"
          title="View Your Bag"
        >
          ...
          <div>
            ...
            <div className="text-sm opacity-90">Your items</div>
          </div>
        </button>
      </motion.div>
    </div>
