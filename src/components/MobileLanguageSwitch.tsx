  ...
  return (
    <div className={`relative ${className}`}>
      ...
      <motion.button
        ...
        whileTap={{ scale: 0.95 }}
        className="flex items-center justify-between w-full bg-white/95 backdrop-blur-xl border border-gray-200/50 rounded-2xl sm:rounded-3xl px-4 py-4 shadow-xl hover:shadow-2xl transition-all"
      >
        <div className="flex items-center space-x-3">
          <div className="w-12 h-12 bg-gradient-to-br from-indigo-500 via-purple-600 to-pink-600 rounded-2xl flex items-center justify-center shadow-lg">
            <Globe className="text-white" size={20} />
          </div>
          <div className="text-left">
            <div className="flex items-center space-x-2">
              <span className="text-2xl">{currentLanguage.flag}</span>
              <span className="font-bold text-gray-900 text-lg">{currentLanguage.nativeName}</span>
            </div>
            <p className="text-sm text-gray-600 font-medium">{currentLanguage.name}</p>
          </div>
        </div>
