...
const RestaurantMenu: React.FC<RestaurantMenuProps> = ({ shop }) => {
  ...
  const [localBag, setLocalBag] = useState<Item[]>([]);
  ...
  const generateWhatsAppInquiry = () => {
    ...
    const inquiryItems = bag.map(item => 
      `• ${item.name} - ${item.price || 'Price on inquiry'}`
    ).join('\n');
    
    const message = `🍽️ Order from ${shop.shopName}
        ...
        {menuCategories.length === 0 ? (
          <div className="text-center py-12">
            ...
            <div className="flex justify-center space-x-4">
              ...
              <a
                href={`https://wa.me/${shop.phone.replace(/[^0-9]/g, '')}?text=${encodeURIComponent(t('restaurant.whatsappMessage', { shopName: shop.shopName }))}`}
                target="_blank"
      ...
      {localBag.length > 0 && (
        ...
        >
          ...
          >
            ...
            <div>
              <div className="font-semibold">{t('restaurant.viewBag')}</div>
              <div className="text-sm opacity-90">
                {localBag.length} {t('restaurant.items')}
              </div>
            </div>
          </button>
        </motion.div>
