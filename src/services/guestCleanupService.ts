...
export const cleanupGuestAccount = async (userId: string, isGuest: boolean): Promise<void> => {
  ...
  try {
    if (process.env.NODE_ENV !== 'production') {
      console.log(`🧹 Starting cleanup for guest account: ${userId}`);
    }
    ...
    if (process.env.NODE_ENV !== 'production') {
      console.log(`✅ Guest account cleanup completed for: ${userId}`);
    }
...
export const scheduleGuestCleanup = async (): Promise<void> => {
  try {
    ...
    if (process.env.NODE_ENV !== 'production') {
      console.log(`🧹 Found ${guestSnapshot.size} abandoned guest accounts for cleanup`);
    }
    ...
    for (const guestDoc of guestSnapshot.docs) {
      try {
        ...
        if (process.env.NODE_ENV !== 'production') {
          console.log(`✅ Cleaned up abandoned guest: ${guestDoc.id}`);
        }
      } catch (error) {
